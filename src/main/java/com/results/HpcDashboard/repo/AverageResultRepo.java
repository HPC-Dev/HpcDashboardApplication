...
public interface AverageResultRepo extends DataTablesRepository<AverageResult, AverageResultId> {
    ...
    public static final String UPDATE_AVG_RESULT = "UPDATE average_result set segment=:segment, avg_result=:avg,per_core_perf=:perCorePerf,perf_per_dollar=:perfPerDollar,perf_per_watt=:perfPerWatt, coefficient_of_variation=:cv, run_count=:count, category=:category,average_perfhib=:averagePerfHIB,perf_per_corehib=:perfPerCoreHIB where bm_name=:bm_name and cpu_sku =:cpu_sku and nodes=:nodes and run_type=:runType";
    ...
    public static final String GET_CPU_CATEGORIES = "select DISTINCT cpu_sku from average_result where category IN (:categories) and nodes=1 ORDER BY cpu_sku ASC";
    ...
    public static final String GET_RUN_TYPES_CATEGORIES = "SELECT  DISTINCT LOWER(run_type) FROM  average_result WHERE app_name=:appName and category IN (:categories) ORDER BY run_type ASC";
    ...
    public static final String GET_CPU_SELECTED_CATEGORIES = "SELECT DISTINCT a.cpu_sku FROM  average_result AS a WHERE  a.app_name = :appName and a.nodes=1 and a.run_type IN (:runTypes) and a.category IN (:categories) ORDER BY cpu_sku ASC";
    ...
    public static final String GET_APP_CATEGORY = "select DISTINCT LOWER(app_name) from average_result where category IN (:categories) ORDER BY app_name ASC;";
    ...
    public static final String GET_SEGMENTS = "select DISTINCT(segment) from average_result ORDER BY category ASC";

    @Modifying
    @Query(value = UPDATE_AVG_RESULT, nativeQuery = true)
    void updateAverageResult(String segment,String bm_name, String cpu_sku, int nodes, double avg,double perCorePerf,double perfPerDollar,double perfPerWatt, double cv, int count, String runType, String category, double averagePerfHIB, double perfPerCoreHIB);
    ...
    @Query(value = GET_APP_CATEGORY, nativeQuery = true)
    List<String> getAPP(String[] categories);
    ...
    @Query(value = GET_RUN_TYPES_CATEGORIES, nativeQuery = true)
    List<String> getRunTypes(String appName, String[] categories);
    ...
    @Query(nativeQuery =true,value = GET_CPU_SELECTED_CATEGORIES)
    List<String> getCpuSelected(String appName,  List<String> runTypes, String[] categories);
    ...
    @Query(value = GET_CPU_CATEGORIES, nativeQuery = true)
    List<String> getCpuCategories(String[] categories);
